...
function GitHubContributions() {
  ...
  const fetchGitHubData = async () => {
    try {
      ...
      // Fetch user stats
      const userResponse = await fetch(`https://api.github.com/users/${mirkashi}}`);
      if (!userResponse.ok) {
      ...
      // Fetch repositories
      const reposResponse = await fetch(`https://api.github.com/users/${mirkashi}/repos?sort=updated&per_page=6`);
      if (!reposResponse.ok) {
